...
watch(computed(()=>store.state.log), logChange)

const menus = [
  {name:"",label:"私人订制",children:[
      {name:"hot",icon:Fire,router:"/hot",label:"热门音乐"},
      {name:"recommend",icon:Creative,router:"/recommend",label:"个性推荐"}]},
  {name:"",label:"资料库",children:[
  ...
  {name:"",label:"播放列表",children:[
      {name:"playlist",icon:MusicList,router:"/playlist",label:"所有播放列表"},
...
<template>
  <div id="menu" >
    <div id="search-input">
      <el-input  placeholder="输入关键词" v-model="keyword" prefix-icon="Search" size="small"
                 @keyup.enter.native="search()" border="false"
                 style="color: #FE668E;margin-bottom: 20px">
      </el-input>
    </div>

    <div v-for="(o,index) in menus">
      <div style="color: #a6a19f;font-size: 10px;font-weight: bold;padding-left: 5px">{{o.label}}</div>
      <div v-for="(c,index) in o.children">
        <div :id=c.name style="padding-left: 15px;border-radius: 5px" @click="chooseMenu(c)">
          <el-button type="text" :icon=c.icon
                     style="color: #ff2a40;">
          </el-button>
          <el-button type="text"
                     style="color: #4f4d4d;font-size: 12px;margin-left: 3px">{{c.label}}
          </el-button>
        </div>
      </div>
    </div>
  </div>
    <div id="message" :style="{'border-top': store.state.log.message ? '1px solid #cfcbc9': '' }">{{store.state.log.message}}</div>
</template>
